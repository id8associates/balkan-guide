/**
 * {COUNTRY_NAME} — Country Page Logic
 * Uses shared global.js helpers: initMap, addMarkers, buildPlaceHTML, openSidePanel, closeSidePanel
 */

// ── Map Setup: center on country, adjust zoom ──
const map = initMap([LATITUDE, LONGITUDE], ZOOM, MIN_ZOOM, MAX_ZOOM);
// Example for Croatia: initMap([45.1, 15.2], 7, 6, 16)
...
function renderSidePanel(placeId) {
  ...
  const html = buildPlaceHTML(place, restaurants);
  openSidePanel(html);
}
